 ...
 */
public class DemoFoneBook {
    private FoneBook myFoneBook = new FoneBook();
    ...
    public void start() {
        ...
        while (choice != 0) {
            switch (choice) {
                case 1 -> {
                    myFoneBook.addContact(getFromConsole("Введите Фамилию Имя Телефоны через пробел"));
                    choice = menu();
                }
                case 2 -> {
                    printMap(myFoneBook.findLastName(getFromConsole("Введите фамилию")));
                    choice = menu();
                }
                case 3 -> {
                    printMap(myFoneBook.getContacts());
                    choice = menu();
                }
                default -> choice = menu();
            }
